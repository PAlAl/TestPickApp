    ...
    fun init(model: PostViewModel, onItemClick: (Int) -> Unit, onExpandCollapseClick: (Int) -> Unit) {
        with(itemView) {
            ...
            if (model.isExpandText) {
                ...
                item_post_expand_collapse_title.text = itemView.context.getText(R.string.posts_menu_item_collapse_title)
                item_post_expand_collapse_img.setImageResource(R.drawable.ic_collapse)
            } else {
                item_post_text.maxLines = defaultTextMaxLines
                item_post_expand_collapse_title.text = itemView.context.getText(R.string.posts_menu_item_expand_title)
